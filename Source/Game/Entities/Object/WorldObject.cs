    ...
    public abstract class WorldObject : WorldLocation, IDisposable
    {
        ...
        public void BuildMovementUpdate(WorldPacket data, CreateObjectBits flags, Player target)
        {
            ...
            if (flags.ActivePlayer)
            {
                ...
                bool HasRuneState = ToUnit().GetPowerIndex(PowerType.RuneBlood) != (int)PowerType.Max;
                ...
                if (HasRuneState)
                {
                    player.Runes.GetRuneData(LoopTime.ServerTime).Write(data);
                }
        ...
        public GameObject FindNearestUnspawnedGameObject(int entry, float range)
        {
                ...
                if (magnet != null)
                {
                    if (spellInfo.CheckExplicitTarget(this, magnet) == SpellCastResult.SpellCastOk && IsValidAttackTarget(magnet, spellInfo))
                    {
                        ...
                        if (spellInfo.HasHitDelay())
                        {
                            ...
                            if (spellInfo.HasAttribute(SpellAttr9.MissileSpeedIsDelayInSeconds))
