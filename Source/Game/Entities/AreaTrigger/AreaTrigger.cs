...
namespace Game.Entities
{
    public class AreaTrigger : WorldObject
    {
        ...
        bool Create(AreaTriggerId areaTriggerCreatePropertiesId, Map map, Position pos, Milliseconds duration, AreaTriggerSpawn spawnData = null, Unit caster = null, Unit target = null, SpellCastVisual spellVisual = default, SpellInfo spellInfo = null, Spell spell = null, AuraEffect aurEff = null)
        {
            ...
            if (IsServerSide())
                SetUpdateFieldValue(areaTriggerData.ModifyValue(m_areaTriggerData.DecalPropertiesID), 24); // Blue decal, for .debug areatrigger visibility
        ...
        float GetScaleCurveProgress(ScaleCurve scaleCurve, Milliseconds timeTo)
        {
            ...
            return Math.Clamp((GetTimeSinceCreated() - scaleCurve.StartTimeOffset) / (float)timeTo, 0.0f, 1.0f);
        }
        ...
        float GetScaleCurveValueAtProgress(ScaleCurve scaleCurve, float x)
        {
            ...
            if ((scaleCurve.ParameterCurve & 1u) != 0)
                return BitConverter.UInt32BitsToSingle((scaleCurve.ParameterCurve & ~1u));
