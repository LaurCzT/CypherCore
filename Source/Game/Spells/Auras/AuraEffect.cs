...
namespace Game.Spells
{
    public class AuraEffect
    {
        ...
        {
            int stackAmountForBonuses = !spellEffectInfo.EffectAttributes.HasFlag(SpellEffectAttributes.SuppressPointsStacking) ? stack : 1;
        ...
        void HandlePhase(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (apply)
                PhasingHandler.AddPhase(target, GetMiscValueB(), true);
            else
                PhasingHandler.RemovePhase(target, GetMiscValueB(), true);
        }
        ...
        void HandlePhaseGroup(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (apply)
                PhasingHandler.AddPhaseGroup(target, GetMiscValueB(), true);
            else
                PhasingHandler.RemovePhaseGroup(target, GetMiscValueB(), true);
        }
        ...
        void HandleAuraModShapeshift(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            int modelid = target.GetModelForForm(form, GetId());
        ...
        void HandleAuraTransform(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (apply)
            {
                ...
                if (transformSpellInfo == null || !GetSpellInfo().IsPositive() || transformSpellInfo.IsPositive())
                {
                    ...
                    {
                        CreatureTemplate ci = Global.ObjectMgr.GetCreatureTemplate(GetMiscValue());
                        if (ci == null)
                        {
                        ...
                        {
                            int model_id = 0;
                            int modelid = ObjectManager.ChooseDisplayId(ci).CreatureDisplayID;
                            if (modelid != 0)
        ...
        void HandleAuraModDisarm(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (target.IsTypeId(TypeId.Player) && !target.IsInFeralForm())
            {
                ...
                if (item != null)
                {
                    var attackType = Player.GetAttackBySlot(slot);
                    ...
                    if (attackType.HasValue)
                    {
                        ...
                        if (!apply) // apply case already handled on item dependent aura removal (if any)
                            player.UpdateWeaponDependentAuras(attackType.Value);
                    }
                }
            }
        ...
        void HandleAuraMounted(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (apply)
            {
                if (mode.HasAnyFlag(AuraEffectHandleModes.SendForClientMask))
                {
                    int creatureEntry = GetMiscValue();
                    int displayId = 0;
                    int vehicleId = 0;
        ...
        void HandleAuraModResistance(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            {                
                ...
                {
                    if (amount == 0)    // "Magic Absorption" spell case (amount depends on player level)
                    {
                        target.StatMods.Update(UnitMods.ResistanceStart + (int)school);
                    }
                    else
                    {
                        target.StatMods.ModifyFlat(UnitMods.ResistanceStart + (int)school, amount, apply, aurApp);
                    }
                }
            }
        }
        ...
        void HandleAuraModBaseResistancePCT(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (!target.IsTypeId(TypeId.Player))
            {
                ...
                {
                    target.StatMods.ModifyMult(UnitMods.Armor, GetAmount(), apply, aurApp);
                }
            }
            else
            {
                for (SpellSchools school = SpellSchools.Normal; school < SpellSchools.Max; school++)
                {
                    if (schoolMask.HasSchool(school))
                    {
                        target.StatMods.ModifyMult(UnitMods.ResistanceStart + (int)school, GetAmount(), apply, aurApp, true);
                    }
                }
            }
        }

        [AuraEffectHandler(AuraType.ModResistanceExclusive)]
        void HandleAuraModResistanceExclusive(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            if (!mode.HasAnyFlag(AuraEffectHandleModes.ChangeAmountMask | AuraEffectHandleModes.Stat))
                return;
        ...
        void HandleModResistancePercent(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            for (SpellSchools school = SpellSchools.Normal; school < SpellSchools.Max; school++)
            {
                if (schoolMask.HasSchool(school))
                {
                    target.StatMods.ModifyMult(UnitMods.ResistanceStart + (int)school, GetAmount(), apply, aurApp);
                }
            }
        }
        ...
        void HandleModBaseResistance(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (!target.IsTypeId(TypeId.Player))
            {
                ...
                if (target.IsPet() && schoolMask.HasSchool(SpellSchools.Normal))
                {
                    target.StatMods.ModifyFlat(UnitMods.Armor, GetAmount(), apply, aurApp, true);
                }
            }
            else
            {
                for (SpellSchools school = SpellSchools.Normal; school < SpellSchools.Max; school++)
                {
                    if (schoolMask.HasSchool(school))
                    {
                        target.StatMods.ModifyFlat(UnitMods.ResistanceStart + (int)school, GetAmount(), apply, aurApp, true);
                    }
                }
            }
        }
        ...
        void HandleModTargetResistance(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (target.IsTypeId(TypeId.Player) && ((SpellSchoolMask)GetMiscValue() & SpellSchoolMask.Normal) != 0)
        ...
        void HandleAuraModStat(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            for (var i = Stats.Strength; i < Stats.Max; i++)
            {
                ...
                if (GetMiscValue() < 0 || GetMiscValue() == (int)i)
                {
                    ...
                    target.StatMods.ModifyFlat(UnitMods.StatStart + (int)i, GetAmount(), apply, aurApp);
                }
            }
        }
        ...
        void HandleModPercentStat(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (GetMiscValue() < -1 || GetMiscValue() >= (int)Stats.Max)
            {
                Log.outError(LogFilter.Spells, "WARNING: Misc Value for AuraType.ModPercentStat not valid");
                return;
            }
            ...
            if (GetMiscValue() == -1)
            {
                ...
                {
                    target.StatMods.ModifyMult(UnitMods.StatStart + (int)i, amount, apply, aurApp, true);
                }
            }
            else
            {
                target.StatMods.ModifyMult(UnitMods.StatStart + GetMiscValue(), amount, apply, aurApp, true);
            }
        }
        ...
        void HandleModTotalPercentStat(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (GetMiscValue() < -1 || GetMiscValue() >= (int)Stats.Max)
            {
                Log.outError(LogFilter.Spells, "WARNING: Misc Value for AuraType.ModTotalStatPercentage not valid");
                return;
            }
            ...
            if (GetMiscValue() == -1)
            {
                for (Stats i = Stats.Strength; i < Stats.Max; i++)
                {
                    target.StatMods.ModifyMult(UnitMods.StatStart + (int)i, amount, apply, aurApp);
                }
            }
            else
            {
                target.StatMods.ModifyMult(UnitMods.StatStart + GetMiscValue(), amount, apply, aurApp);
            }
        }
        ...
        [AuraEffectHandler(AuraType.ModRangedAttackPowerOfStatPercent)]
        void HandleAuraModRangedAttackPowerOfStatPercent(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            if (!mode.HasAnyFlag((AuraEffectHandleModes.ChangeAmountMask | AuraEffectHandleModes.Stat)))
                return;
            ...
            if (target is Player player)
            {
                player.UpdateRangedAttackPowerAndDamage();
            }
        }

        [AuraEffectHandler(AuraType.ModMeleeAttackPowerOfStatPercent)]
        void HandleAuraModMeleeAttackPowerOfStatPercent(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            if (!mode.HasAnyFlag((AuraEffectHandleModes.ChangeAmountMask | AuraEffectHandleModes.Stat)))
                return;
            ...
            if (target is Player player)
            {
                player.UpdateMeleeAttackPowerAndDamage();
            }
        }
        ...
        void HandleOverrideAttackPowerBySpellPower(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            target.UpdateMeleeAttackPowerAndDamage();
            target.UpdateRangedAttackPowerAndDamage();
        }
        ...
        void HandleModPowerRegen(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (target is Player player)
            {
                ...
                if (powerType == PowerType.Runes)
                {
                    player.UpdateAllRunesRegen();
                }
                else
                {
                    player.UpdatePowerRegen(powerType);
                }
            }
        }
        ...
        void HandleTriggerSpellOnHealthPercent(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            int triggerSpell = GetSpellEffectInfo().TriggerSpell;
        ...
        void HandleModSpellCritSchoolChance(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            {
                // Always (schoolMask == schoolMask.Magic) for creatures?
                // target.BaseSpellCritChance += apply ? GetAmount() : -GetAmount();
                Cypher.Assert(false, "Units have only BaseSpellCritChance mod?", "HandleModSpellCritSchoolChance");
            }
        }
        ...
        void HandleAuraModAttackPower(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            target.StatMods.ModifyFlat(UnitMods.AttackPowerMelee, GetAmount(), apply, aurApp);
        }
        ...
        void HandleAuraModRangedAttackPower(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            target.StatMods.ModifyFlat(UnitMods.AttackPowerRanged, GetAmount(), apply, aurApp);
        }
        ...
        void HandleAuraModAttackPowerPercent(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            Unit target = aurApp.GetTarget();
            target.StatMods.ModifyMult(UnitMods.AttackPowerMelee, GetAmount(), apply, aurApp);
        }
        ...
        void HandleAuraModRangedAttackPowerPercent(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            target.StatMods.ModifyMult(UnitMods.AttackPowerRanged, GetAmount(), apply, aurApp);
        }
                        ...
                        case SpellFamilyNames.Generic:
                            switch (GetId())
                            {
                                ...
                                case 42783: // Wrath of the Astromancer
                                    target.CastSpell(target, GetAmount(), new CastSpellExtraArgs(this));
                                    break;
                                ...
                                case 60244:  // Blood Parrot Despawn Aura
                                    target.CastSpell(null, GetAmount(), new CastSpellExtraArgs(this));
                                    break;
        ...
        void HandleAuraLinked(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            int triggeredSpellId = GetSpellEffectInfo().TriggerSpell;
        ...
        void HandleAuraOverrideSpells(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            if (apply)
            {
                ...
                {
                    for (int i = 0; i < SharedConst.MaxOverrideSpell; ++i)
                    {
        ...
        void HandlePeriodicDamageAurasTick(Unit target, Unit caster)
        {
            ...
            switch (GetAuraType())
            {
                    ...
                    if (GetSpellInfo().SpellFamilyName == SpellFamilyNames.Generic)
                    {
                        switch (GetId())
                        {
                            ...
                            case 72856: // Unbound Plague
                                damage *= (int)Math.Pow(1.25f, _ticksDone);
                                break;
                            default:
                                break;
                        }
                ...
                case AuraType.PeriodicDamagePercent:
                    ...
                    damage = (int)Math.Ceiling(MathFunctions.CalculatePct((float)target.GetMaxHealth(), damage));
                    damage = target.SpellDamageBonusTaken(caster, GetSpellInfo(), damage, DamageEffectType.DOT);
                    break;
                default:
                    break;
        ...
        void HandlePeriodicHealthFunnelAuraTick(Unit target, Unit caster)
        {
            ...
            damage = (int)(damage * gainMultiplier);
        ...
        void HandleLinkedSummon(AuraApplication aurApp, AuraEffectHandleModes mode, bool apply)
        {
            ...
            else
            {
                ...
                foreach (int summonEntry in summonedEntries)
                {
    ...
    class AbsorbAuraOrderPred : Comparer<AuraEffect>
    {
        public override int Compare(AuraEffect aurEffA, AuraEffect aurEffB)
        {
            ...
            // Ice Barrier
            if ((int)spellProtoA.GetCategory() == 471)
                return 1;
            if ((int)spellProtoB.GetCategory() == 471)
                return 0;
