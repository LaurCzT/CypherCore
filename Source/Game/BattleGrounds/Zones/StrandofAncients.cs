...
namespace Game.BattleGrounds.Zones
{
    public class BgStrandOfAncients : Battleground
    {
        ...
        bool ResetObjs()
        {
            ...
            int atF = SAMiscConst.Factions[Attackers];
            int defF = SAMiscConst.Factions[Attackers != 0 ? BattleGroundTeamId.Alliance : BattleGroundTeamId.Horde];
            ...
            for (byte i = 0; i < GateStatus.Length; ++i)
                GateStatus[i] = Attackers == BattleGroundTeamId.Horde ? SAGateState.AllianceGateOk : SAGateState.HordeGateOk;
            ...
            //Graveyards
            for (int i = 0; i < SAGraveyards.Max; i++)
            {
            ...
            UpdateWorldState(SAWorldStateIds.RightGyHorde, GraveyardStatus[SAGraveyards.RightCapturableGy] == BattleGroundTeamId.Horde ? 1 : 0);
            UpdateWorldState(SAWorldStateIds.LeftGyHorde, GraveyardStatus[SAGraveyards.LeftCapturableGy] == BattleGroundTeamId.Horde ? 1 : 0);
            UpdateWorldState(SAWorldStateIds.CenterGyHorde, GraveyardStatus[SAGraveyards.CentralCapturableGy] == BattleGroundTeamId.Horde ? 1 : 0);

            UpdateWorldState(SAWorldStateIds.RightGyAlliance, GraveyardStatus[SAGraveyards.RightCapturableGy] == BattleGroundTeamId.Alliance ? 1 : 0);
            UpdateWorldState(SAWorldStateIds.LeftGyAlliance, GraveyardStatus[SAGraveyards.LeftCapturableGy] == BattleGroundTeamId.Alliance ? 1 : 0);
            UpdateWorldState(SAWorldStateIds.CenterGyAlliance, GraveyardStatus[SAGraveyards.CentralCapturableGy] == BattleGroundTeamId.Alliance ? 1 : 0);

            if (Attackers == BattleGroundTeamId.Alliance)
            {
        ...
        public override void PostUpdateImpl(TimeSpan diff)
        {
            ...
            if (Status == SAStatus.Warmup)
            {
                ...
                if (TotalTime >= SATimers.WarmupLength)
                {
                    ...
                    TriggerGameEvent(Attackers == BattleGroundTeamId.Alliance ? 23748 : 21702);
                }
            ...
            if (go != null)
            {
                switch (go.GetGoType())
                {
                    case GameObjectTypes.Goober:
                        if (invoker != null)
                            if (eventId == (int)SAEventIds.BG_SA_EVENT_TITAN_RELIC_ACTIVATED)
                    ...
                    case GameObjectTypes.DestructibleBuilding:
                    {
                        ...
                        if (gate != null)
                        {
                            int gateId = gate.GateId;

                            // damaged
                            if (eventId == go.GetGoInfo().DestructibleBuilding.DamagedEvent)
                            {
                                GateStatus[gateId] = Attackers == BattleGroundTeamId.Horde ? SAGateState.AllianceGateDamaged : SAGateState.HordeGateDamaged;
                                ...
                                PlaySoundToAll(Attackers == BattleGroundTeamId.Alliance ? SASoundIds.WallAttackedAlliance : SASoundIds.WallAttackedHorde);
                            }
                            // destroyed
                            else if (eventId == go.GetGoInfo().DestructibleBuilding.DestroyedEvent)
                            {
                                GateStatus[gate.GateId] = Attackers == BattleGroundTeamId.Horde ? SAGateState.AllianceGateDestroyed : SAGateState.HordeGateDestroyed;
                                ...
                                PlaySoundToAll(Attackers == BattleGroundTeamId.Alliance ? SASoundIds.WallDestroyedAlliance : SASoundIds.WallDestroyedHorde);
        ...
        public override void HandleKillUnit(Creature creature, Unit killer)
        {
            if (creature.GetEntry() == SACreatureIds.Demolisher)
            {
                ...
                if (killerPlayer != null)
                    UpdatePlayerScore(killerPlayer, ScoreType.DestroyedDemolisher, 1);

                int worldStateId = Attackers == BattleGroundTeamId.Horde ? SAWorldStateIds.DestroyedHordeVehicles : SAWorldStateIds.DestroyedAllianceVehicles;
        ...
        void OverrideGunFaction()
        {
            ...
            for (byte i = SACreatureTypes.Gun1; i <= SACreatureTypes.Gun10; i++)
            {
                ...
                if (gun != null)
                    gun.SetFaction(SAMiscConst.Factions[Attackers != 0 ? BattleGroundTeamId.Alliance : BattleGroundTeamId.Horde]);
            }
        ...
        void CaptureGraveyard(int i, Player source)
        {
            ...
            switch (i)
            {
                case SAGraveyards.LeftCapturableGy:
                {
                    ...
                    DelObject(flag);
                    AddObject(flag, (SAMiscConst.ObjEntries[flag] - (teamId == BattleGroundTeamId.Alliance ? 0 : 1)),
                    ...
                    UpdateWorldState(SAWorldStateIds.LeftGyAlliance, GraveyardStatus[i] == BattleGroundTeamId.Alliance ? 1 : 0);
                    UpdateWorldState(SAWorldStateIds.LeftGyHorde, GraveyardStatus[i] == BattleGroundTeamId.Horde ? 1 : 0);

                    Creature creature = source.FindNearestCreature(SharedConst.WorldTrigger, 500.0f);
                    if (creature != null)
                        SendChatMessage(creature, teamId == BattleGroundTeamId.Alliance ? SATextIds.WestGraveyardCapturedA : SATextIds.WestGraveyardCapturedH, source);
                }
                break;
                case SAGraveyards.RightCapturableGy:
                {
                    ...
                    UpdateWorldState(SAWorldStateIds.RightGyAlliance, GraveyardStatus[i] == BattleGroundTeamId.Alliance ? 1 : 0);
                    UpdateWorldState(SAWorldStateIds.RightGyHorde, GraveyardStatus[i] == BattleGroundTeamId.Horde ? 1 : 0);

                    Creature creature = source.FindNearestCreature(SharedConst.WorldTrigger, 500.0f);
                    if (creature != null)
                        SendChatMessage(creature, teamId == BattleGroundTeamId.Alliance ? SATextIds.EastGraveyardCapturedA : SATextIds.EastGraveyardCapturedH, source);
                }
                break;
                case SAGraveyards.CentralCapturableGy:
                {
                    ...
                    UpdateWorldState(SAWorldStateIds.CenterGyAlliance, GraveyardStatus[i] == BattleGroundTeamId.Alliance ? 1 : 0);
                    UpdateWorldState(SAWorldStateIds.CenterGyHorde, GraveyardStatus[i] == BattleGroundTeamId.Horde ? 1 : 0);

                    Creature creature = source.FindNearestCreature(SharedConst.WorldTrigger, 500.0f);
                    if (creature != null)
                        SendChatMessage(creature, teamId == BattleGroundTeamId.Alliance ? SATextIds.SouthGraveyardCapturedA : SATextIds.SouthGraveyardCapturedH, source);
                }
                break;
                default:
                    //ABORT();
        ...
        public override bool IsSpellAllowed(int spellId, Player player)
        {
            switch (spellId)
            {
                case SASpellIds.AllianceControlPhaseShift:
                    return Attackers == BattleGroundTeamId.Horde;
                case SASpellIds.HordeControlPhaseShift:
                    return Attackers == BattleGroundTeamId.Alliance;
    ...
    class BattlegroundSAScore : BattlegroundScore
    {
        ...
        public override int GetAttr1() { return DemolishersDestroyed; }
        public override int GetAttr2() { return GatesDestroyed; }

        int DemolishersDestroyed;
        int GatesDestroyed;
    }
    ...
    {
        public const int AllianceCapturedTitanPortal = 28944;
        public const int HordeCapturedTitanPortal = 28945;

        public const int RoundTwoStartOneMinute = 29448;
        public const int RoundTwoStartHalfMinute = 29449;
    }
    ...
    struct SASpellIds
    {
        public const int TeleportDefender = 52364;
        public const int TeleportAttackers = 60178;
        public const int EndOfRound = 52459;
        public const int RemoveSeaforium = 59077;
        public const int AllianceControlPhaseShift = 60027;
        public const int HordeControlPhaseShift = 60028;
    }

    struct SACreatureIds
    {
        public const int Kanrethad = 29;
        public const int InvisibleStalker = 15214;
        public const int WorldTrigger = 22515;
        public const int WorldTriggerLargeAoiNotImmunePcNpc = 23472;

        public const int AntiPersonnalCannon = 27894;
        public const int Demolisher = 28781;
        public const int RiggerSparklight = 29260;
        public const int GorgrilRigspark = 29262;
    }

    struct SAGameObjectIds
    {
        public const int GateOfTheGreenEmerald = 190722;
        public const int GateOfThePurpleAmethyst = 190723;
        public const int GateOfTheBlueSapphire = 190724;
        public const int GateOfTheRedSun = 190726;
        public const int GateOfTheYellowMoon = 190727;
        public const int ChamberOfAncientRelics = 192549;

        public const int BoatOneA = 208000;
        public const int BoatTwoA = 193185;
        public const int BoatOneH = 193184;
        public const int BoatTwoH = 208001;
    }
    ...
    struct SASoundIds
    {
        public const int GraveyardTakenHorde = 8174;
        public const int GraveyardTakenAlliance = 8212;
        public const int DefeatHorde = 15905;
        public const int VictoryHorde = 15906;
        public const int VictoryAlliance = 15907;
        public const int DefeatAlliance = 15908;
        public const int WallDestroyedAlliance = 15909;
        public const int WallDestroyedHorde = 15910;
        public const int WallAttackedHorde = 15911;
        public const int WallAttackedAlliance = 15912;
    }
    ...
    {
        public const int Timer = 3557;
        public const int AllyAttacks = 4352;
        public const int HordeAttacks = 4353;
        public const int PurpleGate = 3614;
        public const int RedGate = 3617;
        public const int BlueGate = 3620;
        public const int GreenGate = 3623;
        public const int YellowGate = 3638;
        public const int AncientGate = 3849;
        public const int LeftGyAlliance = 3635;
        public const int RightGyAlliance = 3636;
        public const int CenterGyAlliance = 3637;
        public const int RightAttTokenAll = 3627;
        public const int LeftAttTokenAll = 3626;
        public const int LeftAttTokenHrd = 3629;
        public const int RightAttTokenHrd = 3628;
        public const int HordeDefenceToken = 3631;
        public const int AllianceDefenceToken = 3630;
        public const int RightGyHorde = 3632;
        public const int LeftGyHorde = 3633;
        public const int CenterGyHorde = 3634;
        public const int BonusTimer = 3571;
        public const int EnableTimer = 3564;
        public const int AttackerTeam = 3690;
        public const int DestroyedAllianceVehicles = 3955;
        public const int DestroyedHordeVehicles = 3956;
    }
