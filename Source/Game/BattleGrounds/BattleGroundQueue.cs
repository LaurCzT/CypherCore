...
namespace Game.BattleGrounds
{
    public class BattlegroundQueue
    {
        ...
        public GroupQueueInfo AddGroup(Player leader, Group group, Team team, PvpDifficultyRecord bracketEntry, bool isPremade, int ArenaRating, int MatchmakerRating, int arenateamid = 0)
        {
            ...
            //add GroupInfo to m_QueuedGroups
            {
                ...
                if (!m_queueId.Rated && !isPremade && WorldConfig.Values[WorldCfg.BattlegroundQueueAnnouncerEnable].Bool)
                {
                    BattlegroundTemplate bg = Global.BattlegroundMgr.GetBattlegroundTemplateByTypeId(m_queueId.BattlemasterListId);
                    if (bg != null)
                    {
                        ...
                        if (WorldConfig.Values[WorldCfg.BattlegroundQueueAnnouncerPlayerOnly].Bool)
                        {
        ...
        public void RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
        {
            ...
            int index = (group.Team == Team.Horde) ? BattlegroundConst.BgQueuePremadeHorde : BattlegroundConst.BgQueuePremadeAlliance;
            ...
            for (int bracket_id_tmp = (int)BattlegroundBracketId.Max - 1; bracket_id_tmp >= 0 && bracket_id == -1; --bracket_id_tmp)
            {
                ...
                for (int j = index; j < BattlegroundConst.BgQueueTypesCount; j += SharedConst.PvpTeamsCount)
                {
            ...
            Log.outDebug(LogFilter.Battleground, 
                $"BattlegroundQueue: Removing {guid}, from bracket_id {bracket_id}");
            ...
            {
                Battleground bg = Global.BattlegroundMgr.GetBattleground(group.IsInvitedToBGInstanceGUID, m_queueId.BattlemasterListId);
                if (bg != null)
            ...
            if (m_queueId.TeamSize != 0 && m_queueId.Rated && group.Players.Empty() 
                && WorldConfig.Values[WorldCfg.ArenaQueueAnnouncerEnable].Bool)
            {
            ...
            if (group.IsInvitedToBGInstanceGUID != 0 && m_queueId.Rated && decreaseInvitedCount)
            {
                ...
                {
                    Log.outDebug(LogFilter.Battleground, 
                        $"UPDATING memberLost's personal arena rating " +
                        $"for {guid} by opponents rating: {group.OpponentsTeamRating}");
        ...
        bool InviteGroupToBG(GroupQueueInfo ginfo, Battleground bg, Team side)
        {
            ...
            if (ginfo.IsInvitedToBGInstanceGUID == 0)
            {
                ...
                BattlegroundTypeId bgTypeId = m_queueId.BattlemasterListId;
                ...
                foreach (var guid in ginfo.Players.Keys)
                {
                    ...
                    Log.outDebug(LogFilter.Battleground, 
                        $"Battleground: invited player {player.GetName()} ({player.GetGUID()}) to " +
                        $"BG instance {bg.GetInstanceID()} queueindex {queueSlot} bgtype {m_queueId.BattlemasterListId}");
        ...
        */
        void FillPlayersToBG(Battleground bg, BattlegroundBracketId bracket_id)
        {
            ...
            if (WorldConfig.Values[WorldCfg.BattlegroundInvitationType].Int32 == (int)BattlegroundQueueInvitationType.Even)
            {
            ...
            if (WorldConfig.Values[WorldCfg.BattlegroundInvitationType].Int32 == (int)BattlegroundQueueInvitationType.NoBalance)
                return;
            /*
            ...
            */
            ...
            int diffAli = aliFree - m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount();
            int diffHorde = hordeFree - m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount();

            while (Math.Abs(diffAli - diffHorde) > 1 
                && (m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount() > 0 || m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount() > 0))
            {
                ...
                if (diffAli < diffHorde)
                {
                    ...
                    if (m_SelectionPools[BattleGroundTeamId.Alliance].KickGroup(diffHorde - diffAli))
                    {
                        for (; alyIndex < aliCount && m_SelectionPools[BattleGroundTeamId.Alliance].AddGroup(m_QueuedGroups[(int)bracket_id][BattlegroundConst.BgQueueNormalAlliance][alyIndex], (aliFree >= diffHorde) ? aliFree - diffHorde : 0); alyIndex++)
                    ...
                    if (m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount() == 0)
                    {
                        ...
                        m_SelectionPools[BattleGroundTeamId.Horde].KickGroup(diffHorde - diffAli);
                    }
                }
                else
                {
                    //kick horde group, add to pool new group if needed
                    if (m_SelectionPools[BattleGroundTeamId.Horde].KickGroup(diffAli - diffHorde))
                    {
                        for (; hordeIndex < hordeCount && m_SelectionPools[BattleGroundTeamId.Horde].AddGroup(m_QueuedGroups[(int)bracket_id][BattlegroundConst.BgQueueNormalHorde][hordeIndex], (hordeFree >= diffAli) ? hordeFree - diffAli : 0); hordeIndex++)
                            ++hordeIndex;
                    }
                    if (m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount() == 0)
                    {
                        ...
                        m_SelectionPools[BattleGroundTeamId.Alliance].KickGroup(diffAli - diffHorde);
                    }
                }
                //count diffs after small update
                diffAli = aliFree - m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount();
                diffHorde = hordeFree - m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount();
            }
        }
        ...
        bool CheckPremadeMatch(BattlegroundBracketId bracket_id, int MinPlayersPerTeam, int MaxPlayersPerTeam)
        {
            //check match
            if (!m_QueuedGroups[(int)bracket_id][BattlegroundConst.BgQueuePremadeAlliance].Empty() && !m_QueuedGroups[(int)bracket_id][BattlegroundConst.BgQueuePremadeHorde].Empty())
            {
                ...
                if (ali_group != null && horde_group != null)
                {
                    ...
                    int maxPlayers = Math.Min(m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount(), m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount());
        ...
        bool CheckNormalMatch(BattlegroundBracketId bracketId, int minPlayers, int maxPlayers)
        {
            ...
            if (WorldConfig.Values[WorldCfg.BattlegroundInvitationType].Int32 != (int)BattlegroundQueueInvitationType.NoBalance
                && m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount() >= minPlayers 
                && m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount() >= minPlayers)
            {
            ...
            if (Global.BattlegroundMgr.IsTesting() && (m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount() != 0 
                || m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount() != 0))
                return true;
        ...
        bool CheckSkirmishForSameFaction(BattlegroundBracketId bracket_id, int minPlayersPerTeam)
        {
            if (m_SelectionPools[BattleGroundTeamId.Alliance].GetPlayerCount() < minPlayersPerTeam
                && m_SelectionPools[BattleGroundTeamId.Horde].GetPlayerCount() < minPlayersPerTeam)
            {
                return false;
            }

            int teamIndex = BattleGroundTeamId.Alliance;
            int otherTeam = BattleGroundTeamId.Horde;
        ...
        public void BattlegroundQueueUpdate(TimeSpan diff, BattlegroundBracketId bracket_id, int arenaRating)
        {
            ...
            BattlegroundTemplate bg_template = Global.BattlegroundMgr.GetBattlegroundTemplateByTypeId(m_queueId.BattlemasterListId);
            if (bg_template == null)
            {
                Log.outError(LogFilter.Battleground, 
                    $"Battleground: Update: " +
                    $"bg template not found for {m_queueId.BattlemasterListId}");
                return;
            }
            ...
            foreach (var mapId in bg_template.BattlemasterEntry.MapId)
            {
                ...
                var bgQueues = Global.BattlegroundMgr.GetBGFreeSlotQueueStore(mapId);
            ...
            int MinPlayersPerTeam = bg_template.GetMinPlayersPerTeam();
            int MaxPlayersPerTeam = bg_template.GetMaxPlayersPerTeam();

            if (bg_template.IsArena())
            {
                MaxPlayersPerTeam = m_queueId.TeamSize;
                MinPlayersPerTeam = Global.BattlegroundMgr.IsArenaTesting() ? 1 : m_queueId.TeamSize;
            }
            ...
            if (!bg_template.IsArena())
            {
                if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
                {
                    ...
                    {
                        Log.outError(LogFilter.Battleground, 
                            $"BattlegroundQueue.Update - " +
                            $"Cannot create Battleground: {m_queueId.BattlemasterListId}");
                        return;
                    }
            ...
            else if (bg_template.IsArena())
            {
                ...
                if (found == 2)
                {
                    ...
                    Log.outDebug(LogFilter.Battleground, 
                        $"setting oposite teamrating for team {aTeam.ArenaTeamId} to {aTeam.OpponentsTeamRating}");

                    Log.outDebug(LogFilter.Battleground, 
                        $"setting oposite teamrating for team {hTeam.ArenaTeamId} to {hTeam.OpponentsTeamRating}");
        ...
        class SelectionPool
        {
            ...
            public bool AddGroup(GroupQueueInfo ginfo, int desiredCount)
            {
                ...
                if (ginfo.IsInvitedToBGInstanceGUID == 0 && desiredCount >= PlayerCount + ginfo.Players.Count)
                {
                    ...
                    PlayerCount += ginfo.Players.Count;
                    return true;
                }
            ...
            public bool KickGroup(int size)
            {
    ...
    {  
        ...
        public BattlegroundQueueTypeId(BattlegroundTypeId battlemasterListId, byte bgType, bool rated, byte teamSize)
        {
        ...
        {
            return new BattlegroundQueueTypeId((BattlegroundTypeId)(packedQueueId & 0xFFFF), (byte)((packedQueueId >> 16) & 0xF), ((packedQueueId >> 20) & 1) != 0, (byte)((packedQueueId >> 24) & 0x3F));
        }
    ...
    class BGQueueInviteEvent : BasicEvent
    {
        ...
        public override bool Execute(TimeSpan e_time, TimeSpan p_time)
        {
            ...
            var queueSlot = player.GetBattlegroundQueueIndex(m_QueueId);
    ...
    class BGQueueRemoveEvent : BasicEvent
    {
        ...
        public override bool Execute(TimeSpan e_time, TimeSpan p_time)
        {
            ...
            Battleground bg = Global.BattlegroundMgr.GetBattleground(m_BgInstanceGUID, m_BgQueueTypeId.BattlemasterListId);
            ...
            if (queueSlot < SharedConst.PvpTeamsCount)         // player is in queue, or in Battleground
            {
                ...
                {
                    Log.outDebug(LogFilter.Battleground,
                        $"Battleground: removing player {player.GetGUID()} from bg queue " +
                        $"for instance {m_BgInstanceGUID} because of not pressing enter battle in time.");
