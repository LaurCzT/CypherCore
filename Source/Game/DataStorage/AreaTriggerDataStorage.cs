        ...
        public void LoadAreaTriggerTemplates()
        {
...

            {
                ...
                if (!templateActions.IsEmpty())
                {
                    do
                    {
                        AreaTriggerId areaTriggerId = new(templateActions.Read<int>(0), templateActions.Read<bool>(1));
...

            {
                ...
                if (!vertices.IsEmpty())
                {
                    do
                    {
                        AreaTriggerId createPropertiesId = new(vertices.Read<int>(0), vertices.Read<bool>(1));
...

            {
                ...
                if (!areatriggerCreateProperties.IsEmpty())
                {
                    do
                    {
                        ...
                        AreaTriggerId createPropertiesId = new(areatriggerCreateProperties.Read<int>(0), areatriggerCreateProperties.Read<bool>(1));
                        createProperties.Id = createPropertiesId;

                        AreaTriggerId areaTriggerId = new(areatriggerCreateProperties.Read<int>(2), areatriggerCreateProperties.Read<bool>(3));
                        ...
                        createProperties.MoveCurveId = ValidateAndSetCurve(areatriggerCreateProperties.Read<int>(5));
                        createProperties.ScaleCurveId = ValidateAndSetCurve(areatriggerCreateProperties.Read<int>(6));
                        createProperties.MorphCurveId = ValidateAndSetCurve(areatriggerCreateProperties.Read<int>(7));
                        createProperties.FacingCurveId = ValidateAndSetCurve(areatriggerCreateProperties.Read<int>(8));
...

            {
                ...
                if (!circularMovementInfos.IsEmpty())
                {
                    do
                    {
                        AreaTriggerId createPropertiesId = new(circularMovementInfos.Read<int>(0), circularMovementInfos.Read<bool>(1));
        ...
        public void LoadAreaTriggerSpawns()
        {
            ...
            if (!result.IsEmpty())
            {
                do
                {
                    ...
                    AreaTriggerId createPropertiesId = new(result.Read<int>(1), result.Read<bool>(2));
